        ...
        <DrawerItems>
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-1 justify-center">
              <Avatar img={user?.avatar ?? ""} size="md"  rounded/>
              <p className="text-white text-[12px] font-[500]">{user?.name ?? "User"}</p>
            </div>
             <button type="button" onClick={() => { setIsOpen(false); navigate('/user')} } 
              className="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 
              font-medium rounded-full text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
            >
                { 'Edit Profile'}
              </button>
               {
